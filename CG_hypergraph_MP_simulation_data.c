...
#define Q 597
#define m  4
#define Hypergraph 1 /*Hypergraph can be 1 only for Hyperedge=0*/
#define Hyperedge 0 /*Hyperedge=1(0) means hyperedge (node) percolation */
#define Nrunmax 40
...
int main(int argc, char** argv){
   ...
   **************************************************************************/
    ...
    for(i=0;i<N+Q;i++){
     ...
     ffp=fopen("senate_hypergraph.txt", "r");
...
/*%%%%%%%%%MESSAGGE PASSING%%%%%%%%%%%%*/
    gp=fopen("GC_Hypergraph_MP_senate.txt","w");
    for(f=0.;f<1.;f+=0.01){
    p=1-f;
    nsumold1=1000000;
...
/*%%%%%%%%%%%%SIMULATION%%%%%%%%%%%%%%%%%*/
    ...
    gp3=fopen("GC_Hypergraph_simulations_senate.txt","w");
        for (nc=0;nc<100;nc++){
            f=nc*0.01;
